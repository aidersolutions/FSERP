...
__all__ = ['Template', 'Product', 'price_digits', 'IngredientsOfDish']
...
class Template(ModelSQL, ModelView):
    ...
    type = fields.Selection(TYPES, 'Type', required=True, states=STATES,
        depends=DEPENDS)
    ingredient = fields.Boolean('Ingredient', select = True)
...
class IngredientsOfDish(ModelSQL,ModelView):
    "product to prodct"
    __name__ ="product.ingredientsofdish"
    dish = fields.Many2One('product.product', 'Dish', required=False)
    ingredient = fields.Many2One('product.product', 'Ingredient', required=False)
    quantity = fields.Numeric('Quantity',required = False)
    quantity_uom = fields.Many2One('product.uom', 'Quantity UOM', required=False)
...
class Product(ModelSQL, ModelView):
    ...
    cost_price_uom = fields.Function(fields.Numeric('Cost Price',
        digits=price_digits), 'get_price_uom')
    ingredients = fields.Many2Many("product.ingredientsofdish", 'dish','ingredient','ingredients')
    dishes_using = fields.Many2Many("product.ingredientsofdish",'ingredient','dish','dishes_using')
    serve=fields.Numeric('Serve')
